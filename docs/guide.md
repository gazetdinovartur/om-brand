...
**Оплата:** сумма в **копейках** (500000 = 5 000 ₽). После сохранения открывается карточка со **ссылкой для клиента** (копировать). **Ссылка СБП** подставляется из шаблона в настройках (`{amount_rubles}`, `{token}`…). Telegram-уведомление при создании.
